...
class MainFrame(tk.Frame):
    ...
    def _build_ui(self):
        ...
        # ===== Основная область: детали слева, таблица справа =====
        main_area = tk.Frame(self, bg=COLOR_MAIN_BG)
        main_area.pack(fill="both", expand=True, padx=10, pady=5)

        # Левая часть — детали (ширина 300)
        detail_container = tk.Frame(main_area, bg=COLOR_MAIN_BG, width=300)
        detail_container.pack(side="left", fill="y", padx=(0, 10))
        detail_container.pack_propagate(False)

        # Правая часть — таблица
        right_frame = tk.Frame(main_area, bg=COLOR_MAIN_BG)
        right_frame.pack(side="right", fill="both", expand=True)

        v_scroll = tk.Scrollbar(right_frame, orient="vertical")
        h_scroll = tk.Scrollbar(right_frame, orient="horizontal")
        ...
        self.tree = ttk.Treeview(
            right_frame, columns=self.columns, show="tree headings",
        ...
        self.detail_frame = tk.Frame(detail_container, bg="#FFFFFF", relief="solid", bd=1)
        ...
        # Перепаковываем тулбар вниз, main_area — выше него
        toolbar.pack_forget()
